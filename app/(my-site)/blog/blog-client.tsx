            ...
            <div>
                {filtered.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {filtered.toSorted((a, b) => Date.parse(b.publishedAt) - Date.parse(a.publishedAt)).map((post) => (
                            <Link
                                key={post.slug}
                                href={"/blog/" + post.slug}
                            ...
                            >
                                ...
                                <p className="text-sm text-slate-500 mt-1">{new Date(Date.parse(post.publishedAt)).toDateString()}</p>
                            </Link>
                        ))}
                    </div>
                )}
